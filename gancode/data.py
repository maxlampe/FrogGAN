...
class gan_data:
    ...
    def __init__(self, hpar, norm=None, bno_aug=False):
        ...
        if bno_aug:
            ...
            trans.append(transforms.RandomAffine(degrees=15.0, shear=5.0))
            trans.append(transforms.Pad(padding=-10))
            trans.append(
                transforms.RandomResizedCrop(
                    size=self.hpar["image_size"], scale=(0.9, 1.0)
                )
            )
            trans.append(
                transforms.RandomApply([transforms.ColorJitter(0.1, 0.1)], p=0.3)
            )
        ...
        self.dataset = dset.ImageFolder(
            root=self.hpar["dataroot"], transform=transforms.Compose(trans)
        )
