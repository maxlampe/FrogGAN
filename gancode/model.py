...
class Generator(nn.Module):
    ...
    def __init__(self, hpar, ngpu):
        ...
        self.main = nn.Sequential(
            nn.ConvTranspose2d(
                self.hpar["nz"], self.hpar["ngf"] * 8, 4, 1, 0, bias=False
            ),
            nn.BatchNorm2d(self.hpar["ngf"] * 8),
            nn.ReLU(True),
            # state size. (ngf*8) x 4 x 4
            nn.ConvTranspose2d(
                self.hpar["ngf"] * 8, self.hpar["ngf"] * 4, 4, 2, 1, bias=False
            ),
            nn.BatchNorm2d(self.hpar["ngf"] * 4),
            nn.ReLU(True),
            # state size. (ngf*4) x 8 x 8
            nn.ConvTranspose2d(
                self.hpar["ngf"] * 4, self.hpar["ngf"] * 2, 4, 2, 1, bias=False
            ),
            nn.BatchNorm2d(self.hpar["ngf"] * 2),
            nn.ReLU(True),
            # state size. (ngf*2) x 16 x 16
            nn.ConvTranspose2d(
                self.hpar["ngf"] * 2, self.hpar["ngf"], 4, 2, 1, bias=False
            ),
...
class FrogGan:
    ...
    def __init__(
        self, hpar, ngpu, device, bnew_net: bool = True, train_label: str = "tmp"
    ):
        self.hpar = hpar
        self.ngpu = ngpu
        self.device = device
