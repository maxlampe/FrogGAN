...
class GanTrainer:
    ...
    def __init__(self, gan_net, dataloader, h_par_train: dict):
        ...
        self.fixed_noise = torch.randn(
            64, self.h_par_model["nz"], 1, 1, device=self.device
        )
        ...
        self.optimizerD = optim.Adam(
            ...
            lr=self.h_par_train["lr"],
            betas=(self.h_par_train["beta1"], 0.999),
        )
    ...
    def train(self, num_epochs=None, bfid_study: bool = False, bverbose: bool = False):
        ...
        for epoch in range(num_epochs):
            for i, data in enumerate(self.dataloader, 0):
                ...
                if (
                    self.iters < self.h_par_train["max_iter_noise"]
                    and self.h_par_train["bdiscnoise"]
                ):
                ...
                if sigma != 0.0:
                    disc_noise = torch.normal(mean=0.0, std=sigma, size=data[0].shape)
                ...
                if self.h_par_train["bsmoothlabel"]:
                    ...
                    label = torch.full(
                        (b_size,),
                        (self.real_label - self.h_par_train["smoothing"]),
                        dtype=torch.float,
                        device=self.device,
                    )
                else:
                ...
                noise = torch.randn(
                    b_size, self.h_par_model["nz"], 1, 1, device=self.device
                )
                ...
                if sigma != 0.0:
                    disc_noise = torch.normal(mean=0.0, std=sigma, size=data[0].shape)
                    output = self.netD(
                        (fake + disc_noise.to(self.device)).detach()
                    ).view(-1)
                else:
                ...
                if (self.iters % 250 == 0) or (
                    (epoch == num_epochs - 1) and (i == len(self.dataloader) - 1)
                ):
                    with torch.no_grad():
            ...
            if self.fid is not None:
                ...
                if self.fid <= 140.0 and bnew_fid == True:
                    print(f"Found good one FID: {self.fid:0.2f}")
                    self.gan_net.save_models(
                        tag=f"fid{int(self.fid)}_{epoch}", outdir="."
                    )
                    bnew_fid = False
    ...
    def gen_fakes(self, n_gen_images: int = 6, upscale: float = 2.0):
        """"""

        self.netG.eval()
        noise = torch.randn(
            n_gen_images, self.h_par_model["nz"], 1, 1, device=self.device
        )
